...
internal class CustomValidator : IBasicValidator
{
    private readonly Lazy<string> _message;
    private readonly Func<TaskContext, bool> _predicate;

    public CustomValidator(string message, Func<TaskContext, bool> predicate)
    {
        _message = new Lazy<string>(() => message);
        _predicate = predicate;
    }

    public CustomValidator(Lazy<string> message, Func<TaskContext, bool> predicate)
    {
        _message = message;
        _predicate = predicate;
    }

    public string GetName => nameof(CustomValidator);
